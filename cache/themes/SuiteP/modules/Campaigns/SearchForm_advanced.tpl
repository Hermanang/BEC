...
{assign var="id" value=$fields.start_date_advanced.name }

{if isset($smarty.request.start_date_advanced_range_choice)}
{assign var="starting_choice" value=$smarty.request.start_date_advanced_range_choice}
{else}
...
{html_options options=$fields.start_date_advanced.options selected=$starting_choice}
</select>
</div>

<div id="{$id}_range_div" style="{if preg_match('/^\[/', $smarty.request.range_start_date_advanced)  || $starting_choice == 'between'}display:none{else}display:''{/if};">
<input autocomplete="off" type="text" name="range_{$id}" id="range_{$id}" value='{if empty($smarty.request.range_start_date_advanced) && !empty($smarty.request.start_date_advanced)}{$smarty.request.start_date_advanced}{else}{$smarty.request.range_start_date_advanced}{/if}' title=''   size="11" class="dateRangeInput">
...
<input autocomplete="off" type="text" name="start_range_{$id}" id="start_range_{$id}" value='{$smarty.request.start_range_start_date_advanced }' title=''  tabindex='' size="11" class="rangeDateInput">
...
{$APP.LBL_AND}
{assign var=date_value value=$fields.start_date_advanced.value }
<input autocomplete="off" type="text" name="end_range_{$id}" id="end_range_{$id}" value='{$smarty.request.end_range_start_date_advanced }' title=''  tabindex='' size="11" class="dateRangeInput" maxlength="10">
...
{assign var="id" value=$fields.end_date_advanced.name }

{if isset($smarty.request.end_date_advanced_range_choice)}
{assign var="starting_choice" value=$smarty.request.end_date_advanced_range_choice}
{else}
{assign var="starting_choice" value="="}
{/if}
...
<div class="dateTimeRangeChoice" style="white-space:nowrap !important;">
<select id="{$id}_range_choice" name="{$id}_range_choice" onchange="{$id}_range_change(this.value);">
{html_options options=$fields.end_date_advanced.options selected=$starting_choice}
</select>
</div>

<div id="{$id}_range_div" style="{if preg_match('/^\[/', $smarty.request.range_end_date_advanced)  || $starting_choice == 'between'}display:none{else}display:''{/if};">
<input autocomplete="off" type="text" name="range_{$id}" id="range_{$id}" value='{if empty($smarty.request.range_end_date_advanced) && !empty($smarty.request.end_date_advanced)}{$smarty.request.end_date_advanced}{else}{$smarty.request.range_end_date_advanced}{/if}' title=''   size="11" class="dateRangeInput">
    <button id="{$id}_trigger" type="button" onclick="return false;" class="btn btn-danger"><span class="suitepicon suitepicon-module-calendar"  alt="{$APP.LBL_ENTER_DATE}"></span></button>
&nbsp;(<span class="dateFormat">{$USER_DATEFORMAT}</span>)
<script type="text/javascript">
Calendar.setup ({ldelim}
...
<input autocomplete="off" type="text" name="start_range_{$id}" id="start_range_{$id}" value='{$smarty.request.start_range_end_date_advanced }' title=''  tabindex='' size="11" class="rangeDateInput">
    <button id="start_range_{$id}_trigger" type="button" onclick="return false" class="btn btn-danger"><span class="suitepicon suitepicon-module-calendar" alt="{$APP.LBL_ENTER_DATE}"></span></button>
&nbsp;(<span class="dateFormat">{$USER_DATEFORMAT}</span>)
<script type="text/javascript">
Calendar.setup ({ldelim}
...
{$APP.LBL_AND}
{assign var=date_value value=$fields.end_date_advanced.value }
<input autocomplete="off" type="text" name="end_range_{$id}" id="end_range_{$id}" value='{$smarty.request.end_range_end_date_advanced }' title=''  tabindex='' size="11" class="dateRangeInput" maxlength="10">
    <span class="suitepicon suitepicon-module-calendar" id="end_range_{$id}_trigger" alt="{$APP.LBL_ENTER_DATE}"></span>
&nbsp;(<span class="dateFormat">{$USER_DATEFORMAT}</span>)
<script type="text/javascript">
Calendar.setup ({ldelim}
...
{html_options id='campaign_type_advanced' name='campaign_type_advanced[]' options=$fields.campaign_type_advanced.options size="6" class="templateGroupChooser" multiple="1" selected=$fields.campaign_type_advanced.value}
        ...
        <div class="">
            ...
            <div class="col-xs-12">
                                <label for='assigned_user_id_advanced'>{sugar_translate label='LBL_ASSIGNED_TO' module='Campaigns'}</label>
                            </div>
            <div class="form-item">
